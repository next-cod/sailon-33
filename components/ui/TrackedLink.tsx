...
export function TrackedLink({ event, children, variant = "primary", arrow = false, className = "", ...props }: Props) {
  ...
  return (
    <a
      {...props}
      className={`inline-flex min-h-12 items-center justify-center gap-2 rounded-full px-6 py-3 text-[15px] font-bold transition-transform duration-200 hover:-translate-y-0.5 active:translate-y-0 ${variants[variant]} ${className}`}
      onClick={(e) => {
        props.onClick?.(e);
