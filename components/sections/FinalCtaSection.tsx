    ...
    <section className="bg-white pb-0 pt-12">
      <div className="container-shell">
        <Reveal className="relative overflow-hidden rounded-[32px] bg-[var(--signal)] p-7 text-[var(--forest-deep)] shadow-[0_24px_70px_rgba(95,198,70,.24)] sm:p-12 lg:p-16">
          ...
          <div className="relative max-w-5xl"><h2 className="section-title balance">Посмотрите, как Сэйлон будет работать именно с вашими клиентами</h2><p className="mt-6 max-w-3xl text-lg leading-relaxed text-[var(--forest-deep)]/72">Месяц, чтобы собрать настройки, подключить первый канал и проверить реальные диалоги без автоматических списаний</p><div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center"><TrackedLink href={siteLinks.signup} event="final_signup_click" variant="light" className="!bg-[var(--forest-deep)] !text-white hover:!bg-[var(--forest)]">Попробовать месяц бесплатно <ArrowRight size={18}/></TrackedLink><span className="flex items-center gap-2 text-sm font-semibold"><Check size={17}/>После теста — оплата по счёту</span></div></div>
        </Reveal>
      </div>
    </section>
  );
