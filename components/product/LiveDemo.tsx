  ...
  return (
    <div className="mt-12 overflow-hidden rounded-[30px] bg-[var(--forest-deep)] p-4 text-white shadow-soft sm:p-7">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={scenario.id} id={`demo-panel-${scenario.id}`} role="tabpanel" aria-labelledby={`demo-tab-${scenario.id}`} aria-live="polite" initial={reduced ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -8 }} className="mt-2 grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
          <div className="rounded-[24px] bg-[var(--paper)] p-5 text-[var(--ink)] sm:p-7">
            ...
            <div className="mt-5 grid gap-3">{scenario.messages.map(([side, text], index) => <motion.div key={`${side}-${index}`} initial={reduced ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .08 }}><ChatBubble side={side as "client" | "assistant"}>{text}</ChatBubble></motion.div>)}</div>
          </div>
